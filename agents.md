...
## (중요) UI/UX·디자인 구현 가이드(4.pint 준용)

> 목표: 비개발자도 **3초 안에 “지금 상태”와 “다음 행동 1개”**를 이해할 수 있게 만든다.

- **UI/UX는 기능 구현이 목적이 아니다**
  - 화면은 항상 “지금 어떤 상태인지”와 “이제 뭘 하면 되는지”가 먼저 보이게 한다.
  - 설명이 길어지면 `자세히`/툴팁/접기로 숨긴다(기본 1~2줄).
- **마이크로카피(문구) 규칙**
  - 기본 말투는 **`~해요체`**로 통일한다(약관/공지 같은 예외만 `~입니다체`).
  - 버튼 라벨은 **동사 포함 2~4단어**로 쓴다(예: `다시 시도`, `목록에 추가`, `순서 저장`).
  - 한 문장에 “상태/원인/요청”을 섞지 않는다(한 문장=한 메시지).
  - 번역투/과장/템플릿 문구를 피하고, 사용자가 얻는 결과를 먼저 쓴다.
- **개발자 용어/내부값 노출 금지(리뷰 블로커)**
  - 화면에 `id/job/queue/progress/api/error/timeout/http 500` 같은 표현이 보이면 버그다.
  - 이 프로젝트에서는 특히 **roomId/userId 같은 숫자 식별자**를 기본 화면에 노출하지 않는다.
  - 정말 필요하면 `자세히 보기`에서만 제공하고 **`복사` 버튼**을 함께 둔다.
- **상태/에러/빈 화면(Empty) 규칙**
  - 상태는 사용자 언어로: `가져오는 중/완료/문제가 생겼어요`처럼 표현한다.
  - 에러는 **1줄 요약 + 1줄 다음 행동(버튼)**만(원문 에러/스택트레이스 금지).
  - 빈 화면은 “없음”으로 끝내지 말고 **상태 설명 + 다음 행동**을 함께 제공한다.
- **레이아웃/가독성 체크(필수)**
  - 정보 우선순위(요약 카드 → 리스트/테이블 → 상세)를 지킨다.
  - 모바일(360) / 태블릿(768) / 데스크톱(1280)에서 레이아웃 파손/가로 흔들림이 없어야 한다.
  - 긴 텍스트는 `min-w-0` + 말줄임/줄바꿈으로 깨짐을 방지한다.
  - 터치 타겟은 최소 44px(버튼/토글/칩) 기준으로 만든다.
- **Next.js 구현 원칙(성능/체감)**
  - 가능하면 `page.tsx`는 서버 컴포넌트로 유지하고, 클라이언트 로직은 `*.client.tsx`로 분리한다.
  - 무거운 모달/패널은 눌렀을 때만 로드/마운트하고, 첫 클릭에 “무반응”처럼 보이지 않게 로딩 표시를 둔다.
  - 리스트/테이블의 row 링크는 기본 `prefetch={false}`로 두고, prefetch를 켤 땐 실제 체감이 좋아지는지 확인한다.
  - 서버 `page.tsx`에서 `searchParams` 의존이 커져 첫 진입이 느려지면 “고정된 기본 화면(쉘) + 클라이언트에서 파라미터 동기화”로 전환한다.
  - 큰 데이터는 1번에 다 받기보다 **chunk로 나눠 먼저 받은 것부터** 화면에 반영한다(“아무 것도 안 뜸” 시간 줄이기).
  - 조회 실패를 `200 + 빈 배열`로 숨기지 말고 `ok=false` + 5xx로 응답해 “없음”과 “장애”를 구분한다.

## (중요) Fallback 금지(근본 해결 우선, 4.pint 준용)

> 핵심 원칙: “대충 보여주기”는 금지. 데이터가 없으면 **데이터가 채워지게** 고친다.

- **금지(리뷰 블로커)**:
  - 운영 UI에서 `어떤 분/미확인` 같은 이름 폴백으로 **데이터 미완전을 가리기**
  - 해시/토큰/암호문처럼 보이는 값(예: base64/hex)을 **닉네임인 것처럼 노출**
  - `roomId/userId` 같은 **숫자 식별자 노출**(직접/간접 포함)
- **올바른 처리**:
  - 데이터가 없으면 `null`로 두고, UI는 **“확인 중/불러오는 중”** 상태를 명확히 표시한다.
  - 누락이 반복되면 수동 백필/버튼으로 때우지 말고 **수집/복호화/동기화 파이프라인**을 고쳐 자동으로 채운다.
  - (예외) **카카오 발신 메시지**에서는 userId 노출 방지를 위해 닉네임이 없을 때만 “어떤 분” 같은 완곡 표현을 허용한다(위 “발신 메시지 UX 가드레일” 준수).

---

## (필수) 작업 완료 시 문서 업데이트/보고 규칙(4.pint 준용)

- “작업 완료” 보고 전, 변경 범위에 맞춰 아래 문서를 최신화한다.
  - 규칙/운영 변경: `agents.md`, `docs/adr/*`
  - 작업과 직접 관련된 문서: `docs/reference/*`, `docs/workflow/*`, `docs/ops/*`
- 최종 보고에는 아래를 반드시 포함한다.
  - 업데이트한 파일 경로 목록
  - 변경 이유 + 핵심 변경점 요약

## (필수) PR 코멘트/머지 규칙(4.pint 준용)

- PR 코멘트가 달리면 아래 순서를 지킨다.
  1. 코멘트 반영(코드/설명/검증/템플릿)
  2. 스레드에 회신(무엇을 어떻게 반영했는지)
  3. “Resolved/완료” 확인 후 머지
- 코멘트 미반영 상태에서 머지하지 않는다.
- (권장) 리뷰 요청을 했다면, 리뷰 결과를 읽고 **반영/미반영 결정과 근거**를 PR에 남긴다.

## (필수) PR 올리는 방법(이 워킹트리 예외 포함)

- 원칙(권장): `feat/*|fix/*|chore/*` 브랜치에서 Draft PR → 검증 후 main 머지
- 단, 이 PC의 `C:\\dev\\12.kakao`는 **공유 워킹트리(main-only 운영 예외)**라서,
  여기서는 브랜치 체크아웃/`git pull/merge/rebase`를 하지 않는다.
- 이 워킹트리에서 PR을 올릴 때(권장 패턴):
  1. `main`에서 변경을 **작은 커밋 단위로** 만든다.
  2. 커밋을 패치로 뽑는다: `git format-patch -1 <commit> -o C:\\dev\\patches_<name>`
  3. 별도 clone/worktree에서 브랜치를 만들고 패치를 적용한다.
     - 안전한 clone(Windows 예약 파일명 충돌 방지): `git clone --no-checkout <repo> <dir>` → `git checkout -f main`
     - 브랜치: `git switch -c feat/<name>`
     - 적용: `git am <patch>`
  4. 푸시/PR 생성: `git push -u origin <branch>` + `gh pr create ...`
  5. 머지 후 브랜치/임시 worktree 정리까지 포함해 “작업 종료”로 본다.
...
- 부분 재기동(권장):
  ...
  - course-membership-audit-worker(선택 기능): `windows/start_course_membership_audit_worker.ps1 -Restart`
    ...
    - **ACTIONS 탭(운영)**: 코스 스프레드시트 `ACTIONS`는 “할 일 큐” 단일 표다.  
      - 컬럼: `우선순위` / `해야 할 일` / `사유` / `대상` / `필요한 방` / `바꿀 닉네임` / `현재 닉네임`
      - `우선순위`는 `지금/오늘/확인/정리`이며, 위에서부터 처리하면 된다.
      - `바꿀 닉네임`이 `<이름마스킹>(카페닉)`처럼 보이면, 이름 마스킹 규칙(첫 글자 + @ 반복 + 마지막 글자)을 적용해 완성하면 된다.
    - **시트 정리(중요)**: derived 탭(`OVERVIEW`/`ACTIONS`)은 매 갱신마다 **시트 전체 서식까지 리셋**해서,
      아래쪽에 예전 서식(검은 줄/색상/merge)이 남지 않게 한다. (`values.clear`만으로는 부족)
...
- 멤버 DB가 비어 권한 판별이 불가능한 경우:
  ...
  - `scripts/openchat_load_members.ps1`를 **자동 트리거**해(송신 없이) 단말에서 멤버 목록을 스크롤 로딩하여 DB를 채운다.
    - 주의: open.kakao join scheme(`kakaoopen://join?...&r=...`)은 adb shell에서 `&`가 백그라운드 연산자로 해석될 수 있어 `\\&` 이스케이프가 필요하다(스크립트에서 자동 처리). 미적용 시 KakaoTalk 진입 실패 → IRIS 이미지 발신 실패(Welcome 템플릿 이미지 미발송)로 이어질 수 있다.
  - 레이트리밋:
    - roomId 기준 15분 쿨다운
...
## 9. UI/UX 구현 지침(4.pint agents.md 차용)

> 목적: 운영진(비개발자)도 **첫 화면 1초 내** “무슨 페이지인지/다음에 뭘 눌러야 하는지” 이해하게 만든다.

- 페이지는 Server-first
  - `page.tsx`에 `'use client'`를 붙여 SSR을 포기하지 않는다.
  - 클라이언트 로직/상태는 `ui/*` 또는 `*.client.tsx`로 분리한다.
- 첫 화면은 항상 “보이는 UI”부터
  - 네트워크/스냅샷 대기 때문에 **화면 전체가 비거나(빈 화면) 스피너만** 보이는 구조를 피한다.
  - 최소한의 헤더/탭/검색/기본 안내는 즉시 렌더한다.
- 무거운 UI는 Lazy
  - Drawer/Modal/차트/테이블 상세 등은 **열릴 때만 렌더**한다(초기 mount 비용 최소화).
  - 첫 클릭이 먹통처럼 느껴지지 않게 **즉시 반응(버튼 disabled/로딩 표시)**을 제공한다.
- 실패를 “성공처럼” 숨기지 않는다
  - API/스냅샷 실패를 `200 OK + 빈 배열`로 숨기지 말고, `ok=false` + 5xx로 명확히 구분한다.
  - 실패 응답은 `Cache-Control: no-store`로 캐시되지 않게 한다.
- 전환/중복요청을 관리한다
  - 페이지/탭 전환 중 in-flight 요청은 가능하면 `AbortSignal`로 취소해 낭비를 줄인다.
  - `setTimeout`/polling 사용 시 cleanup을 확실히 해 타이머 누적을 방지한다.

---

## 10. PR/리뷰 운영 지침(4.pint agents.md 차용)

- PR 올린 직후 10분 내(가능하면) 체크리스트/요약 코멘트를 남긴다.
  - 변경 파일 경로 목록
  - 변경 이유 + 사용자 영향(UX/운영)
  - 검증 방법/실행한 명령어(해당되는 것만)
- 리뷰는 Conversation이 아니라 Files changed 기준으로 확인한다.
- 리뷰 코멘트 처리 순서(필수)
  1. 코멘트 반영(코드/문서/테스트)
  2. PR 코멘트로 “무엇을/왜/어떻게” 반영했는지 답변
  3. 해당 스레드를 Resolved
- `@codex`로 리뷰를 요청했다면
  - codex 리뷰 결과가 달리기 전에는 머지하지 않는다.
  - 반영 후 `@codex`로 반영 코멘트를 남기고, 해결(Resolved) 여부를 확인한다.
- 중요(이 워킹트리 예외): `C:\\dev\\12.kakao`는 공유 워킹트리(main-only)이므로
  - 여기서는 PR 브랜치 생성/체크아웃을 하지 않는다.
  - PR이 필요하면 별도 clone/worktree에서 브랜치 워크플로로 진행한다.
