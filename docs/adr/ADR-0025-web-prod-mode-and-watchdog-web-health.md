...
- watchdog는 API `/status`와 별개로 Web health(`http://127.0.0.1:<WebPort>/api/ping`)를 체크한다.
...
4. **자동 복구는 명시적 근거 기반**: Web은 `/api/ping`(200) 실패를 근거로만 재시작한다.
...
- `windows/watchdog.ps1`
  - Web health(`/api/ping`) 연속 실패 시 Web만 자동 재시작
