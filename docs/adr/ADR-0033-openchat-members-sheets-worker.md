...
- **알림(테스트 방)**: 실패/스킵 시 테스트용 오픈채팅방(`18462226881291012`)으로 1회 알림을 발신한다(전제: `safeMode=false`, `talkApi.enabled=true`).
...
- **재시도 없음 + 테스트방 알림**: upsert 실패/스킵 발생 시 즉시 재시도하지 않으며, 테스트용 오픈채팅방(`18462226881291012`)으로 1회 알림을 보낸다.
