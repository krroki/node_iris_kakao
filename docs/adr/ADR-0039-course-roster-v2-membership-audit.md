...
4. **결제 SSOT(Google Sheets)를 read-only로 연동**한다. (선택)
   ...
   - 운영자가 결제 시트 `닉네임` 칼럼에 `이전닉->현재닉`(또는 `→`) 형태로 기록한 경우,
     워커는 `현재닉`을 기준으로 표시/점검하고 `이전닉`은 매칭(alias) 용도로만 사용한다.
...
- 카페 크롤링 재사용:
  ...
  - `ACTIONS`: 단일 표 “할 일 큐” + `우선순위(지금/오늘/확인/정리)` 컬럼(해야 할 일/사유/대상/필요한 방/바꿀 닉네임/현재 닉네임)
- 상태/락:
