...
- **실패 시 자동 재시도(Briefing Studio)**
  - `command-worker`는 Briefing Studio 발송이 실패하면 **잠시 후 자동 재시도**한다(백오프).
  - 최종 실패(재시도 초과)일 때만 `ack(ok=false)`로 실패를 확정하고 운영 알림을 보낸다(노이즈 방지).
- **재시도 중복 방지(jobId idempotency)**
  - `/send/pint/briefing_bundle`는 `jobId`(prod) 기준으로 **중복 발송을 방지**한다.
  - 부분 성공(이미지 OK, 텍스트 실패) 이후 재시도 시에는 **이미지를 재발송하지 않고 텍스트만 재시도**한다.
  - 상태 파일: `node-iris-app/data/pint_briefing_bundle_state.json` (TTL 기본 7일)
- **운영 알림 방 분리**
