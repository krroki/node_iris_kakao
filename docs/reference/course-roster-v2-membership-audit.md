...
> **운영 UI 안내(중요)**: 현업 운영은 스프레드시트가 아니라 **CourseOps 웹 콘솔(go.yoorang.kr, ACTIONS 중심)**로 전환한다.  
> 이 문서는 “v2 워커/시트(레거시/백업)”의 데이터 구조와 동작을 설명한다.  
> - 웹 콘솔: `docs/reference/course-ops-v2-web-console.md` (ADR-0046)
...
- `ACTIONS`
  ...
  - 구조(현재):
    - 상단: `총/입장 안내/닉네임 변경/결제 확인/카페닉 확인/권한 정리/데이터 점검` 요약
    - 단일 표(필터로 정리):
      - 컬럼: `우선순위(지금/오늘/확인/정리)` / `해야 할 일` / `사유` / `대상` / `필요한 방` / `바꿀 닉네임` / `현재 닉네임`
      - `현재 닉네임`은 한 줄로 요약되어(`·` 구분) 보여서 화면이 덜 지저분하다.
      - `바꿀 닉네임`은 가능하면 결제 SSOT의 `성함`을 기반으로 `정@@록(카페닉)` 형태까지 구체 추천한다.
        - 성함이 없거나(또는 매칭 불가) `<이름마스킹>(카페닉)`처럼 보이면, **이름 마스킹 규칙(첫 글자 + @ 반복 + 마지막 글자)**을 적용해 완성하면 된다.
  - v2 워커가 매번 **clear + 전체 재작성**한다(derived view).
