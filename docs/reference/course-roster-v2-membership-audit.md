...
- `OVERVIEW`
  - 운영자가 “오늘 봐야 할 것”을 바로 볼 수 있도록 만든 **요약/액션 시트**
  - 포함(예시):
    - 운영자 액션 카드(필수방 미참여/닉네임 변경 요청/일반반 프리미엄 참여/중복/매칭불가)
    - 멤버별 방 참여 현황(필수방 미참여/중복/DB미완전 + 일반반 프리미엄 참여(권한 확인))
    - 톡방-카페 매칭/닉네임 이슈 요약(현재)
    - 닉네임 변경 요청(참여 인식됨)
    - 톡방 닉네임 매칭 불가
    - 카페 명단에 없는 카페닉(톡방 닉네임 기준)
  - 표기(예시):
    - `✅ 참여` / `❌ 미참여(필수)` / `✅ 정상(비대상)` / `⚠️ 참여(비정상)` / `⏳ DB미완전`
  - v2 워커가 매번 **clear + 전체 재작성**한다(derived view).

### 7.0.1) ACTIONS 탭(OVERVIEW 다음, 상세 할일)

- `ACTIONS`
  - 운영자가 “지금 당장 해야 할 일”을 **우선순위(P0~P3) + 카테고리**로 상세히 보는 탭
  - 포함(예시):
    - 지금 처리할 일 요약(건수/설명)
    - 데이터 준비(톡방 DB 미완전 여부/갱신 필요)
    - 참여 누락(필수방 미참여) 상세 리스트
    - 동명이인/중복 매칭(AMBIGUOUS) 상세 리스트
    - 권한 이상(일반반 프리미엄방 참여) 리스트
    - 닉네임 변경 요청(참여는 인식됨) / 매칭 불가 / 카페 명단 불일치 상세 리스트
  - v2 워커가 매번 **clear + 전체 재작성**한다(derived view).
