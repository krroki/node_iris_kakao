...
- `GEMINI_WEB_REUSE_CONTEXT`
  - 기본값: `false`
  - 기본은 작업이 끝나면 Playwright 브라우저 컨텍스트를 닫아 **Chrome 창이 남지 않게** 한다(운영 중 “창이 열려 있는 상태” 유지 방지).
  - 속도가 더 필요하면 `true`로 켜서 (슬롯별) 브라우저 프로세스를 재사용해 **요청마다 Chrome을 다시 띄우는 비용을 줄일 수 있다**(특히 `!사진수정` 체감 속도 개선).
