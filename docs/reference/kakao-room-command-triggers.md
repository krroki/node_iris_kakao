...
>   대신 **테스트용 오픈채팅방(`18462226881291012`)에만** `[운영 알림]` 형태로 요약 알림을 남긴다(운영자 알림).
...
- 멤버 DB가 아직 로딩되지 않은 방(특히 대형방)은 권한 확인이 실패할 수 있다.
  ...
  - 또한 **권한 판별이 필요한 시점(관리자 명령 시)** 멤버 DB가 비어있으면 `scripts/openchat_load_members.ps1`를 자동 트리거(송신 없이 화면 탭/스크롤)하여 DB를 채운다.
    - 레이트리밋:
      - roomId 기준 15분 쿨다운
      - 전역 2분 쿨다운(동시 실행/ADB 충돌 방지)
    - 진단/상태 로그는 테스트용 오픈채팅방으로만 발신한다(운영방 오염 방지).
    - 주기 스캔/갱신에서 방 전체를 대상으로 연쇄 실행하지 않는다(운영 알림 폭주 방지).
