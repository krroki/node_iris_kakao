...
- 시나리오:
  ...
  3. 첫 이미지 처리 분기:
     - 오픈프로필(닫기 안내 대상)이면: 감사 Reply 없이 “프로필 닫기 안내 멘션 + 가이드 이미지(1장)”가 발신되고, 닫힘이 감지되면 확인 멘션이 1회 발신됨(ADR-0045)
     - 오픈프로필이 아니고 기본닉이며 `welcome.followUp.nicknameChangeAfterImage.enabled=true`면:
       - 첫 이미지에 “닉네임 변경 요청” Reply(type=26) 발신
       - 요청 발신 시점부터 15분(`confirmWindowMs`) 내 닉변이 확인되면, Reply가 아닌 **일반 멘션**으로 마무리 1회 발신
     - 그 외(비기본닉): 이미지 메시지에 **답장(Reply)** 으로 랜덤 문구(예: “감사합니다~ 이제 편하게 소통해주시면 됩니다!”)를 1회 발신
- 체크:
  - 같은 사용자가 추가 이미지를 보내도 **추가 답장은 없어야 함**(1회 트리거)
  - 15분이 지난 뒤 첫 이미지를 보내면 **답장이 없어야 함**
  - 15분이 지나도 첫 이미지가 없으면 **추가 발신이 없어야 함**(미업로드 경고 제거, ADR-0045)
  - 기본닉 케이스에서는 “닉변 완료”가 확인되기 전에는 감사 Reply/마무리 멘션이 발신되면 안 됨(요청 이전 `feedType=2` 캐시 오판 방지)
