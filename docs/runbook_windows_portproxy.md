...
> **현재 단말 IP 메모 (2025-11-02)**: Galaxy S9+ (Hyper-V/Redroid) 가상 단말의 Wi-Fi IP는 `192.168.0.216`. 변경되면 `adb -s <USB_SERIAL> shell ip -f inet addr show wlan0` 또는 `adb shell getprop dhcp.wlan0.ipaddress`로 재확인 후 아래 명령들의 `-Device` 인자(IP)도 함께 업데이트하세요.
...
./fix_adb_forward.ps1 -Device 192.168.0.216:5555 -LocalPort 3000 -RemotePort 3000 -AdbPath "$env:USERPROFILE\scrcpy\scrcpy-win64-v3.1\adb.exe"
```
