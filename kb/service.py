...
def chat_summary(req: ChatSummaryRequest):
    ...
    lines: list[str] = [
        ...
        "- 타임스탬프/로그 원문 인용(예: [2025-...], 발신자: 원문) 금지.",
        "- userId(숫자) 노출 금지: 296043063 같은 숫자를 그대로 쓰지 말고 닉네임이 없으면 '어떤 분'으로 처리.",
...
def chat_qa(req: ChatQaRequest):
    ...
    if not picked_idx:
        ...
        if keywords or wants_link:
            ...
            hint = f"{requester_disp}아쉽게도 대화에서 **{kw_hint}** 관련 얘기는 아직 못 찾았어요 😥"
    ...
    if wants_link:
        ...
        if urls:
            answer_lines: list[str] = [
                f"{requester_disp}찾아봤어요! 관련 링크는 아래에 정리해둘게요 😊",
                "",
                "💡 요약 내용",
                "- 대화에서 확인된 링크만 모았어요.",
                "",
    ...
    lines: list[str] = [
        ...
        "",
        "답변 스타일(튜브렌즈 스타일, 엄격 준수):",
        "- 두괄식 결론 + 공감: 첫 줄에서 결론을 말하고, 짧게 공감해줘(😊/😥). (단순히 '찾아봤어요/요약해드릴게요'로만 시작하면 안 됨)",
        "- 구조화된 본문: 정보가 2개 이상이면 번호(1. 2. 3.) 또는 불릿(-)로 끊어서 써줘.",
        "- 시각적 환기: 문단 시작/강조에 이모지(📌, 💡, 😥 등)를 적절히 써줘.",
        "- 모바일 최적화 줄바꿈: 한 문장/한 포인트가 끝나면 줄바꿈을 넣어줘.",
        "- 링크는 본문에 섞지 말고, 맨 아래 푸터로 분리해줘(구분선 `---` 아래).",
        "",
        "금지(무조건):",
        "- '답변:', '근거(Evidence)', '다음 액션(Next Action)', '참고 로그' 같은 섹션/헤더 출력 금지.",
        "- 타임스탬프/로그 원문 인용(예: [2025-...], 발신자: 원문) 금지.",
        "- userId(숫자) 노출 금지: 296043063 같은 숫자를 그대로 쓰지 말고 닉네임이 없으면 '어떤 분'으로 처리.",
        "- 대화 로그에 없는 사실/링크/수치 생성 금지.",
        "- 개인정보(전화번호/이메일/계좌 등) 노출 금지.",
        "",
        "답변 템플릿(상황별):",
        "CASE 1) 정보가 전혀 없을 때:",
        ...
        "- 첫 줄에서 '아쉽게도 ~~는 못 찾았어요 😥'처럼 핵심 누락을 말해줘.",
        "- 다음은 `💡 요약 내용` 아래에 확인된 것만 1~3개 포인트로 정리해줘.",
        "- 마지막 줄은 `📌 정확한 건 공지/운영진 확인이 빠를 것 같아요!`처럼 짧게 마무리해줘.",
        "",
        f"방 이름: {req.room_name or req.room_id}",
        f"질문: {question}",
        "--- 대화 로그 ---",
...
def _postprocess_chat_answer(answer: str) -> str:
    ...
    for raw in s.splitlines():
        ...
        if re.match(r"^\s*[-*]\s*\[(20\d{2}[-./]\d{1,2}[-./]\d{1,2}[^\]]*|\d{1,2}:\d{2}(?::\d{2})?)\]\s*[^:]+:\s*.+$", raw):
            continue
        # 캡처/첨부 메타 라인 제거(예: [2025-12-17 19 42 00.png 352x476])
        if re.match(r"^\s*\[[^\]]+\.(png|jpg|jpeg|gif|webp)\s+[0-9]{2,4}x[0-9]{2,4}\]\s*$", raw, re.I):
            continue
        ...
        line = re.sub(r"\b\d{6,}\b\s*님(?=\S)", "어떤 분", line)
        line = re.sub(r"\b\d{6,}\b\s*님\b", "어떤 분", line)
        line = re.sub(r"\b\d{6,}\b\s*님(?=(이|가|은|는|을|를|에게|한테|에서|도|만|과|와|랑|으로|로|께|부터|까지))", "어떤 분", line)
    ...
    # 링크 섹션이 "없음"만 있는 경우는 푸터 자체를 제거한다.
    # (스타일 SSOT: 링크가 있을 때만 푸터를 보여준다.)
    out = re.sub(r"(?ms)\n*\n---\n🔗\s*관련\s*링크\s*\n\s*(없음|없어요|없습니다)\s*\n*$", "", out)
    out = re.sub(r"(?ms)\n*\n🔗\s*관련\s*링크\s*\n\s*(없음|없어요|없습니다)\s*\n*$", "", out)

    # 푸터가 남았는데 URL이 하나도 없으면 제거(모델 실수 방지).
    if re.search(r"(?m)^---\s*$", out) and re.search(r"(?m)^🔗\s*관련\s*링크\s*$", out):
        after = re.split(r"(?m)^---\s*$", out, maxsplit=1)
        if len(after) == 2:
            tail = after[1]
            # tail에 URL이 없으면 푸터 제거
            if not re.search(r"https?://", tail):
                out = after[0].rstrip()

    # 첫 줄 뒤에 바로 본문이 붙는 경우(모바일 가독성) 빈 줄 1개를 강제한다.
    out = re.sub(r"(?m)^([^\n]+)\n(💡\s*요약\s*내용)\b", r"\1\n\n\2", out)
    out = re.sub(r"(?m)^([^\n]+)\n(-\s+)", r"\1\n\n\2", out)
