...
def run(kind: str):
    stale_sec = int(os.getenv("KB_EMBED_LOCK_STALE_SEC", str(3 * 60 * 60)) or str(3 * 60 * 60))
    with lock_scope(f"embed_{kind}", stale_sec=stale_sec) as acquired:
        if not acquired:
            print(f"[emb] {kind} already running; skip")
            return
