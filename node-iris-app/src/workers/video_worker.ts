...
import { generateGeminiWebVideo, initGeminiWebVideoSessionInteractive } from "../services/geminiWebVideo";
...
type VideoCommand = { kind: "generate"; prompt: string };
...
const logger = new Logger("video-worker");

const STATE_PATH = path.join(APP_ROOT, "data", "video_worker_state.json");
const STATUS_PATH = path.join(APP_ROOT, "data", "video_worker_status.json");
const RUNTIME_PATH = path.join(APP_ROOT, "config", "runtime.json");
const LOCK_PATH = path.join(APP_ROOT, "data", "locks", "video_worker.lock");
const LOGS_DIR = (() => {
  const override = String(process.env.IRIS_LOGS_DIR || "").trim();
  return override ? path.resolve(override) : path.join(APP_ROOT, "data", "logs");
})();

// 운영/진단/알림 로그는 테스트용 방으로만 발신한다.
...
type ActiveJob = { roomId: string; kind: VideoCommand["kind"]; startedAt: number; slotId: string };
const activeJobs: ActiveJob[] = [];

let runtimeCache: { at: number; data: RuntimeConfig } | null = null;
const RUNTIME_CACHE_MS = 1500;
const STREAM_TTL_MS = Number.parseInt(String(process.env.VIDEO_WORKER_STREAM_TTL_MS || "").trim(), 10) || 60_000;
const MAX_CONCURRENCY = (() => {
  const raw = String(process.env.VIDEO_WORKER_MAX_CONCURRENCY || "").trim();
...
const QUEUE_MAX = (() => {
  const raw = String(process.env.VIDEO_WORKER_QUEUE_MAX || "").trim();
  const n = Number.parseInt(raw, 10);
  return Number.isFinite(n) && n > 0 ? n : 15;
})();
const LOG_SEARCH_MAX_BYTES = (() => {
  const raw = String(process.env.VIDEO_WORKER_LOG_SEARCH_MAX_BYTES || "").trim();
  const n = Number.parseInt(raw, 10);
...
async function acquireSingletonLock(): Promise<void> {
  try {
    ...
    if (code !== "EEXIST") {
      logger.warn("[lock] lock 파일 생성 실패 (중복 실행 방지 비활성)", { lockPath: LOCK_PATH, err: String(e) });
      return;
    }
    ...
    if (oldPid && isPidAlive(oldPid)) {
      logger.warn("[lock] 다른 video-worker가 이미 실행 중이므로 종료합니다.", { pid: oldPid, lockPath: LOCK_PATH });
      process.exit(0);
    }
    ...
    try {
      await fs.writeFile(LOCK_PATH, String(process.pid), { encoding: "utf8", flag: "wx" });
      logger.warn("[lock] stale lock 정리 후 획득", { stalePid: oldPid, lockPath: LOCK_PATH });
    } catch (e2) {
      logger.warn("[lock] lock 획득 실패로 종료합니다.", { lockPath: LOCK_PATH, err: String(e2) });
      process.exit(1);
    }
  }
}
  ...
  try {
    ...
    try {
      ...
      if (e && String(e.code || "") !== "ENOENT") {
        console.error("[video-worker] cleanup tmp failed:", e);
      }
    }
  }
}
...
function computeVideoRooms(runtime: RuntimeConfig): string[] {
  const allowedRooms = Array.isArray(runtime.allowedRoomIds)
    ? runtime.allowedRoomIds.map((x) => safeString(x)).filter(Boolean)
    : [];
  return allowedRooms.filter((rid) => isRoomAllowed(runtime, rid) && isFeatureEnabled(runtime, rid, "videoGen"));
}

function parseVideoCommand(text: string): VideoCommand | null {
  ...
  for (const re of variants) {
    ...
    return { kind: "generate", prompt };
  }
  return null;
}

function formatTubeLens(firstLine: string, bullets: string[], links?: string[]): string {
  const out: string[] = [];
  ...
  const body = out.join("\n").trim();
  const urlLines = (links || []).map((u) => safeString(u)).filter(Boolean);
  if (urlLines.length === 0) return body;
  return `${body}\n---\n🔗 관련 링크\n${urlLines.join("\n")}`.trim();
}
...
function isIgnoredSender(entry: StreamEntry): boolean {
  ...
  if (sid === "434886784") return true; // IRIS 봇 계정
  if (sn === "iris") return true;
  return false;
}

async function sendOpsLog(text: string): Promise<void> {
  if (!text.trim()) return;
  const rt = await loadRuntime(true);
  if (!isRoomAllowed(rt, OPS_LOG_ROOM_ID)) return;
  if (isSafeModeOn(rt)) return;
  await tryServerIrisReplyText(logger, OPS_LOG_ROOM_ID, text, 12_000);
}
  ...
  if (isTalkApiEnabled(runtime)) {
    ...
    if (srcLinkId) {
      const attachment: Record<string, unknown> = {
        ...
        src_type: safeString(opts.srcType),
        src_message: safeString(opts.srcMessage),
      };
      const ok = await tryServerTalkApiDispatchRaw(logger, roomId, replyText, 26, attachment, 12_000);
      if (ok) return true;
    }
  }

  // fallback: IRIS text (no reply/mention)
  return await tryServerIrisReplyText(logger, roomId, replyText, 12_000);
}

function computeQueuePositions(roomId: string): { globalPos: number; roomPos: number } {
  const rid = safeString(roomId);
  const activeGlobal = activeJobs.length;
  const activeRoom = activeJobs.filter((j) => safeString(j.roomId) === rid).length;
  const queuedForRoom = JOB_QUEUE.filter((j) => safeString(j.roomId) === rid).length;
  return { globalPos: activeGlobal + JOB_QUEUE.length + 1, roomPos: activeRoom + queuedForRoom + 1 };
}
...
function isRetryableGeminiVideoError(err: unknown): boolean {
  const msg = String((err as any)?.message || err || "");
  const needles = [
    "file_input_not_found",
    "gemini_web_video_eval_timeout",
    "gemini_web_video_overall_timeout",
    "gemini_web_video_no_video_found",
    "gemini_web_video_url_not_found",
    "gemini_web_video_lock_busy",
  ];
...
async function generateGeminiWebVideoWithRetry(opts: {
  prompt: string;
  inputImageBase64?: string | null;
  sessionId: string;
}): Promise<string[]> {
  try {
    return await generateGeminiWebVideo(opts);
  } catch (e) {
    if (!isRetryableGeminiVideoError(e)) throw e;
    logger.warn("[video] gemini retry", { sessionId: opts.sessionId, err: String(e) });
...
async function handleGenerate(
  ...
  if (!promptClean) {
    await sendReplyToMessage({
      ...
      replyText: formatTubeLens("프롬프트가 비어 있어요 😥", [
        "사용법: `!영상 <만들고 싶은 영상 설명>`",
        "예시: `!영상 껌이 화내는 5초 애니메이션 영상 만들어줘`",
        "예시: (이미지에 답장) `!영상 이 이미지로 5초 영상 만들어줘`",
        "예시: (URL 포함) `!영상 https://... 이 이미지로 5초 영상 만들어줘`",
      ]),
      srcLogId: src.logId,
  ...
  if (inlineUrl) {
    try {
      inputImageBase64 = await downloadUrlAsBase64(inlineUrl, 30_000);
    } catch {
      await sendReplyToMessage({
        runtime,
        roomId,
        replyText: formatTubeLens("이미지 로드에 실패했어요 😥", [
          "URL이 공개된 이미지인지 확인해 주세요.",
          "이미지는 최대 12MB까지만 지원해요.",
        ]),
        srcLogId: src.logId,
        srcUserId: src.userId,
        srcType: src.type,
    ...
    // Reply 상태라면: 답장한 이미지(또는 URL)를 찾아 이미지→영상 입력으로 사용
    const rec = await findRawLogRecordByMessageId(roomId, src.logId);
    const repliedLogId = extractReplySrcLogIdFromAttachment(rec?.payload?.attachment);
    if (repliedLogId) {
      const orig = await findRawLogRecordByMessageId(roomId, repliedLogId);
      let imageUrls = extractImageUrlsFromAttachment(orig?.payload?.attachment);
      if (imageUrls.length === 0) {
        imageUrls = extractUrlsFromText(safeString(orig?.snapshot?.messageText));
      }
      if (imageUrls.length === 0) {
        await sendReplyToMessage({
          runtime,
          roomId,
          replyText: formatTubeLens("답장한 이미지를 찾지 못했어요 😥", [
            "이미지(또는 이미지 URL)에 답장으로 `!영상 <프롬프트>`를 보내주세요.",
            "예시: (이미지에 답장) `!영상 이 이미지를 5초 영상으로 만들어줘`",
            "예시: (URL 포함) `!영상 https://... 이 이미지로 5초 영상 만들어줘`",
          ]),
          srcLogId: src.logId,
          srcUserId: src.userId,
      ...
      try {
        inputImageBase64 = await downloadUrlAsBase64(imageUrls[0], 30_000);
      } catch {
        await sendReplyToMessage({
          runtime,
          roomId,
          replyText: formatTubeLens("답장한 이미지 로드에 실패했어요 😥", [
            "이미지가 접근 가능한 상태인지 확인해 주세요.",
            "이미지는 최대 12MB까지만 지원해요.",
          ]),
          srcLogId: src.logId,
          srcUserId: src.userId,
  ...
  await sendReplyToMessage({
    runtime,
    roomId,
    replyText: formatTubeLens("영상 만들기 시작했어요 😊", [
      `요청자: ${senderName || "어떤 분"}`,
      inputImageBase64 ? "답장한 이미지를 기반으로 만들고 있어요." : "텍스트 프롬프트로 만들고 있어요.",
      "완료되면 링크로 보내드릴게요.",
      "(현재 카톡 ‘동영상 첨부’ 형태로 바로 올리기는 미지원이에요.)",
    ]),
    srcLogId: src.logId,
    srcUserId: src.userId,
    srcType: src.type,
  ...
  const urls = await generateGeminiWebVideoWithRetry({ prompt: promptClean, inputImageBase64, sessionId: slotId });
  const url = urls.map((u) => safeString(u)).find(Boolean) || "";
  if (!url) throw new Error("video_url_empty");

  await sendReplyToMessage({
    runtime,
    roomId,
    replyText: formatTubeLens(
      "영상이 준비됐어요! 😊",
      [`요청자: ${senderName || "어떤 분"}`, "아래 링크에서 확인해 주세요.", "(현재 카톡 ‘동영상 첨부’ 형태로 바로 올리기는 미지원이에요.)"],
      [url],
    ),
    srcLogId: src.logId,
    srcUserId: src.userId,
  ...
  try {
    ...
    const launchJob = (job: QueuedJob): boolean => {
      ...
      const p = (async () => {
        ...
        if (!isRoomAllowed(runtime, job.roomId) || isSafeModeOn(runtime)) return;
        if (!isFeatureEnabled(runtime, job.roomId, "videoGen")) {
          await sendReplyToMessage({
            runtime,
            roomId: job.roomId,
            replyText: formatTubeLens("이 방에서는 영상 생성이 꺼져 있어요 😥", [
              "대시보드에서 `영상 생성(!영상)` 토글을 켜주세요.",
              "켜지면 `!영상`으로 다시 요청해 주세요.",
            ]),
        ...
        try {
          ...
          logger.warn("[video] job failed", { roomId: job.roomId, slotId, err: String(e) });
          await sendOpsLog(`[ops] video-worker 실패 [slot ${slotId}]: ${String(e)}`);
          const rt = await loadRuntime();
          await sendReplyToMessage({
            runtime: rt,
            roomId: job.roomId,
            replyText: formatTubeLens("영상 처리에 실패했어요 😥", [
              "잠시 후 다시 시도해 주세요.",
...
async function processEntry(ent: StreamEntry, lastSeenMsRef: { v: number }): Promise<void> {
  ...
  const cmd = parseVideoCommand(text);
  if (!cmd) return;
  ...
  if (!isRoomAllowed(runtime, roomId) || isSafeModeOn(runtime)) {
    await sendOpsLog("[ops] video-worker skip: room not allowed or SAFE_MODE (roomId masked)");
    return;
  }

  if (!isFeatureEnabled(runtime, roomId, "videoGen")) {
    await sendReplyToMessage({
      runtime,
      roomId,
      replyText: formatTubeLens("이 방에서는 영상 생성이 꺼져 있어요 😥", [
  ...
  if (activeGlobal + JOB_QUEUE.length >= QUEUE_MAX) {
    await sendReplyToMessage({
      runtime,
      roomId,
      replyText: formatTubeLens("지금은 요청이 많아요 😥", [
        "잠시 후 다시 시도해 주세요.",
        "대기열이 줄어들면 자동으로 처리돼요.",
      ]),
      srcLogId,
  ...
  if (!willStartImmediately) {
    await sendReplyToMessage({
      ...
      replyText: formatTubeLens("요청 접수했어요 😊", [
        `대기열: 방 ${pos.roomPos}번째 (전체 ${pos.globalPos}번째)`,
        "순서가 되면 자동으로 처리할게요.",
      ]),
      srcLogId,
      srcUserId: senderId,
...
async function connectAndRun(): Promise<void> {
  ...
  while (true) {
    const runtime = await loadRuntime();
    const rooms = computeVideoRooms(runtime);
    if (rooms.length === 0) {
      logger.warn("[stream] videoGen-enabled rooms empty; sleeping");
...
async function main() {
  ...
  if (process.argv.includes("--init-gemini-session")) {
    ...
    await initGeminiWebVideoSessionInteractive(sid);
    return;
  }
