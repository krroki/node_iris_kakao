...
export async function GET() {
  const root = repoRoot();
  const statusPath = path.join(root, "node-iris-app", "data", "course_membership_audit_worker_status.json");
  const statePath = path.join(root, "node-iris-app", "data", "course_membership_audit_worker_state.json");
  const lockPath = path.join(root, "node-iris-app", "data", "locks", "course_membership_audit_worker.lock");
