...
// 캐시 유효 시간 (5분)
// NOTE: 캐시는 /api/device/repair 호출 시 갱신됨. 주기적 갱신 필요 시 별도 스케줄러 추가 필요.
...
async function readDeviceStatus(): Promise<StageStatus> {
  ...
  try {
    ...
    if (ageMs > DEVICE_CACHE_TTL_MS) {
      // 캐시가 오래됨
      stage.ok = false;
      stage.detail = `단말 상태 캐시가 ${Math.round(ageMs / 60000)}분 전에 갱신됨. 수동 복구를 실행하세요.`;
...
async function readBotStatus(): Promise<StageStatus> {
  ...
  try {
    ...
    const tsDate = new Date(lastEventTs);
    const ageMs = Date.now() - tsDate.getTime();
    const healthy = ageMs < 90_000; // 90초 미만이면 정상
    stage.ok = healthy;
