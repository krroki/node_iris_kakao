...
$outLog = Join-Path $LogDir 'auto_faq_worker.out.log'
$errLog = Join-Path $LogDir 'auto_faq_worker.err.log'

$statusPath = Join-Path $botDir 'data\auto_faq_worker_status.json'
$workerEntry = Join-Path $botDir 'dist\workers\auto_faq_worker.js'
...
function Find-WorkerProcs {
  try {
    ...
    $relRe = 'dist[/\\]workers[/\\]auto_faq_worker\.js'
...
if ($workerProcs.Count -gt 1) {
  ...
  Write-Host ("[auto-faq-worker] WARN: multiple worker processes detected: {0}" -f $pidsText) -ForegroundColor Yellow
...
if ($mainPid) {
  if (-not $Restart) {
    Write-Host ("[auto-faq-worker] already running pid={0}; skip start (use -Restart to force)" -f $mainPid) -ForegroundColor Green
    return
  }
  Write-Host ("[auto-faq-worker] restart requested; stopping existing pid={0}" -f $mainPid) -ForegroundColor Yellow
  try { Stop-Process -Id $mainPid -Force -ErrorAction SilentlyContinue } catch {}
  Start-Sleep -Milliseconds 300
}

Write-Host '[auto-faq-worker] ensuring deps' -ForegroundColor Cyan
if (-not (Test-Path 'node_modules')) {
  $ciLog = Join-Path $LogDir ("auto_faq_worker.npm.ci." + (Get-Date -Format 'yyyyMMddHHmmss') + ".log")
...
function Build-IfNeeded {
  ...
  if (-not $needBuild) {
    Write-Host '[auto-faq-worker] dist up-to-date; skip build' -ForegroundColor Yellow
    return $true
  }

  $buildLog = Join-Path $LogDir ("auto_faq_worker.build." + (Get-Date -Format 'yyyyMMddHHmmss') + ".log")
  try { Remove-Item -Force -ErrorAction SilentlyContinue $buildLog } catch {}
  Write-Host ("[auto-faq-worker] building -> {0}" -f $buildLog) -ForegroundColor Cyan

  $npmPath = $null
  try { $npmPath = (Get-Command npm.cmd -ErrorAction Stop).Source } catch {
    Write-Error "[auto-faq-worker] npm not found in PATH"
    return $false
  }
  & $npmPath run --silent build *> $buildLog 2>&1
  $rc = $LASTEXITCODE
  if ($rc -ne 0) {
    Write-Error ("[auto-faq-worker] build failed (exit {0}) - see {1}" -f $rc, $buildLog)
    return $false
  }
  if (-not (Test-Path $workerEntry)) {
    Write-Error ("[auto-faq-worker] build completed but dist entry missing: {0}" -f $workerEntry)
    return $false
  }
  return $true
}
...
if (-not $buildOk) {
  Write-Host '[auto-faq-worker] aborting start due to build failure' -ForegroundColor Red
  exit 1
}

Write-Host "[auto-faq-worker] starting (timeout ${TimeoutSec}s)" -ForegroundColor Green
...
$alive = $null -ne (Get-Process -Id $workerPid -ErrorAction SilentlyContinue)
if ($ready) {
  Write-Host ("[auto-faq-worker] READY pid={0}" -f $workerPid) -ForegroundColor Green
} elseif ($alive) {
  Write-Host ("[auto-faq-worker] STARTED pid={0} (status not ready). See logs {1} / {2}" -f $workerPid, $outLog, $errLog) -ForegroundColor Yellow
} else {
  Write-Host ("[auto-faq-worker] FAILED (process exited). See logs {0} / {1}" -f $outLog, $errLog) -ForegroundColor Red
  exit 1
}
