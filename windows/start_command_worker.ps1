...
$outLog = Join-Path $LogDir 'command_worker.out.log'
$errLog = Join-Path $LogDir 'command_worker.err.log'

$statusPath = Join-Path $botDir 'data\command_worker_status.json'
$workerEntry = Join-Path $botDir 'dist\workers\command_worker.js'
...
function Find-WorkerProcs {
  try {
    ...
    $relRe = 'dist[/\\]workers[/\\]command_worker\.js'
...
if ($workerProcs.Count -gt 1) {
  ...
  Write-Host ("[command-worker] WARN: multiple worker processes detected: {0}" -f $pidsText) -ForegroundColor Yellow
...
if ($mainPid) {
  if (-not $Restart) {
    Write-Host ("[command-worker] already running pid={0}; skip start (use -Restart to force)" -f $mainPid) -ForegroundColor Green
    return
  }
  Write-Host ("[command-worker] restart requested; stopping existing pid={0}" -f $mainPid) -ForegroundColor Yellow
  try { Stop-Process -Id $mainPid -Force -ErrorAction SilentlyContinue } catch {}
  Start-Sleep -Milliseconds 300
}

Write-Host '[command-worker] ensuring deps' -ForegroundColor Cyan
if (-not (Test-Path 'node_modules')) {
  $ciLog = Join-Path $LogDir ("command_worker.npm.ci." + (Get-Date -Format 'yyyyMMddHHmmss') + ".log")
...
function Build-IfNeeded {
  ...
  if (-not $needBuild) {
    Write-Host '[command-worker] dist up-to-date; skip build' -ForegroundColor Yellow
    return $true
  }

  $buildLog = Join-Path $LogDir ("command_worker.build." + (Get-Date -Format 'yyyyMMddHHmmss') + ".log")
  try { Remove-Item -Force -ErrorAction SilentlyContinue $buildLog } catch {}
  Write-Host ("[command-worker] building -> {0}" -f $buildLog) -ForegroundColor Cyan

  $npmPath = $null
  try { $npmPath = (Get-Command npm.cmd -ErrorAction Stop).Source } catch {
    Write-Error "[command-worker] npm not found in PATH"
    return $false
  }
  & $npmPath run --silent build *> $buildLog 2>&1
  $rc = $LASTEXITCODE
  if ($rc -ne 0) {
    Write-Error ("[command-worker] build failed (exit {0}) - see {1}" -f $rc, $buildLog)
    return $false
  }
  if (-not (Test-Path $workerEntry)) {
    Write-Error ("[command-worker] build completed but dist entry missing: {0}" -f $workerEntry)
    return $false
  }
  return $true
}
...
if (-not $buildOk) {
  Write-Host '[command-worker] aborting start due to build failure' -ForegroundColor Red
  exit 1
}

Write-Host "[command-worker] starting (timeout ${TimeoutSec}s)" -ForegroundColor Green
...
$alive = $null -ne (Get-Process -Id $workerPid -ErrorAction SilentlyContinue)
if ($ready) {
  Write-Host ("[command-worker] READY pid={0}" -f $workerPid) -ForegroundColor Green
} elseif ($alive) {
  Write-Host ("[command-worker] STARTED pid={0} (status not ready). See logs {1} / {2}" -f $workerPid, $outLog, $errLog) -ForegroundColor Yellow
} else {
